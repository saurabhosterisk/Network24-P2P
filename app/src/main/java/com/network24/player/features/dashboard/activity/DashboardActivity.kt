...
class DashboardActivity : BaseActivity() {
    companion object {
        ...
        // Fallback matching only, when the exact package name above isn't
        // found - a Cinema Pro 3 build with a different applicationId would
        // still contain one of these in its package name or app label.
        private val CINEMA_PRO_3_NAME_HINTS = listOf("cpro3", "cinemapro", "cinema pro")
    }
    ...
    // Falls back to a name match when the exact package above isn't
    // installed - covers a Cinema Pro 3 build published under a different
    // applicationId. Only ever runs after the exact lookup already failed,
    // so this stays a rare, one-off scan rather than routine work.
    private fun findCinemaPro3ByNameHint(): Intent? {
        ...
        val match = candidates.firstOrNull { resolveInfo ->
            val packageName = resolveInfo.activityInfo?.packageName?.lowercase() ?: return@firstOrNull false
            val label = resolveInfo.loadLabel(packageManager)?.toString()?.lowercase() ?: ""

            CINEMA_PRO_3_NAME_HINTS.any { hint ->
                packageName.contains(hint.replace(" ", "")) || label.contains(hint)
            }
        } ?: return null
